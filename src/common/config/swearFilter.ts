...
[
		...
		/(a|4|\@)+(\s|\W)*s(\s|\W)*s+(\s|\W)*h+(\s|\W)*(o|0)+(\s|\W)*(e|3)*((l|i|1|!)+)(e|3)*/gi,

		// Censor permutations of "bastard".
		/b+((\s|\W)*(a|4|@))+((\s|\W)*(s|z|5))+((\s|\W)*t)+((\s|\W)*(a|4|@|e|3))+((\s|\W)*r)+((\s|\W)*d)+/gi,

		// Censor permutations of "bitch".
		/b+((\s|\W)*(i|l|y|1|!))+(\s|\W)*t+(\s|\W)*c+(\s|\W)*h+/gi,

		// Censor permutations of "prick".
		/(\b|[^a-z0-9])p+((\s|\W)*r)+((\s|\W)*(i|l|1|!))+(\s|\W)*c+((\s|\W)*k)+/gi,

		// Censor permutations of "twat".
		/t+((\s|\W)*w)+((\s|\W)*(a|4|@))+((\s|\W)*t)+((\s|\W)*(s|z|\d))*\b/gi,

		// Censor permutations of "dick".
		/d+((\s|\W)*(i|l|1))+((\s|\W)*c)+((\s|\W)*k)+/gi,

		// Censor permutations of "pussy".
		/p+((\s|\W)*u)+((\s|\W)*(s|5|\$))+(\s|\W)*y+/gi,

		// Censor permutations of "cock".
		/(\b|[^a-z0-9])c+((\s|\W)*(o|0))+((\s|\W)*c)+((\s|\W)*k)+(s|z|(h\w*)|\d)*\b/gi,

		// Censor permutations of "cunt".
		/(c|k)+((\s|\W)*u)+((\s|\W)*n)+(\s|\W)*t+/gi,

		// Censor permutations of "shit".
		/(s|5|\$)+((\s|\W)*h)+((\s|\W)*i)+((\s|\W)*t)+/gi,
		/(\b|[^a-z0-9])(s|5|\$)+((\s|\W)*h)+((\s|\W)*(i|l|a|y|1|!))+((\s|\W)*t)+/gi,
